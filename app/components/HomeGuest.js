    ...
    switch (action.type) {
      ...
      case 'passwordImmediately':
        ...
        if (draft.password.value.length > 50) {
          draft.username.hasErrors = true
          draft.username.message = 'Password cannot exceed 50 characters.'
        }
        return
      case 'passwordAfterDelay':
        if (draft.password.value.length < 12) {
          draft.username.hasErrors = true
          draft.username.message = 'Password must be at least 12 characters.'
        }
        return
      case submitForm:
        if (!draft.username.hasErrors && draft.username.isUnique && !draft.email.hasErrors && draft.email.isUnique && draft.password.isUnique) {
          draft.submitCount++
        }
        return
    }
