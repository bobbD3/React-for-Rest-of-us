...
function ViewSinglePost() {
      ...
      try {
        ...
        if (response.data) {
          ...
          if (appState.user.username != response.data.author.username) {
            appDispatch({ type: 'flashMassage', value: 'You do not have permission to edit that post' })
    ...
    if (state.sendCount) {
      ...
      async function fetchPost() {
        try {
          ...
          appDispatch({ type: 'flashMassage', value: 'Post was updated.' })
        } catch (e) {
          console.log('There was a problem or the request was cancelled.')
        }
      }
  ...
  return (
    <Page title='Edit Post'>
      <Link className='small font-weight-bald' to={`/post/${state.id}`}>
        &laquo;Back to posts
      </Link>
